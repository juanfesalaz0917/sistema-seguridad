...
const DigitalSignaturePage: React.FC = () => {
    ...
    const fetchData = async (id: number) => {
        try {
            ...
            // El backend puede devolver 'photo' o 'photo_url' según la implementación.
            const photoField =
                signatureData?.photo || (signatureData as any)?.photo_url;
            const constructedUrl = photoField
                ? digitalSignatureService.getImageUrl(photoField)
                : '';
            // Debug info to help diagnose why image may not appear
            console.debug(
                'DigitalSignature.fetchData -> signatureData:',
                signatureData,
            );
            console.debug(
                'DigitalSignature.fetchData -> photoField:',
                photoField,
            );
            console.debug(
                'DigitalSignature.fetchData -> constructedUrl:',
                constructedUrl,
            );
    ...
    const handleCancel = () => {
        setIsEditing(false);
        setSelectedFile(null);
        const photoField = signature?.photo || (signature as any)?.photo_url;
        if (photoField) {
            setPreviewUrl(digitalSignatureService.getImageUrl(photoField));
        } else {
            setPreviewUrl('');
        }
    ...
    return (
        <div className="container mx-auto px-4 py-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                <h2 className="text-2xl font-bold text-black mb-0">
                    {user?.name} - Signature
                </h2>
                ...
                <div className="p-6">
                    <div className="flex flex-col lg:flex-row">
                        ...
                        <div className="lg:w-1/2 flex flex-col items-center justify-center lg:border-r lg:border-gray-200 lg:pr-6 mb-6 lg:mb-0">
                            <h5 className="text-gray-600 mb-4 text-lg font-medium">
                                Signature
                            </h5>
                            ...
                            >
                                {previewUrl ? (
                                    <img
                                        ...
                                        onError={(e) => {
                                            console.error(
                                                'DigitalSignature: failed to load image',
                                                previewUrl,
                                                e,
                                            );
                                            // clear preview to show fallback UI
                                            setPreviewUrl('');
                                        }}
                                    />
                                ) : (
                                ...
                                >
                                    <div className="flex items-center">
                                        ...
                                        <span className="text-yellow-800">
                                            Este usuario aún no tiene una firma
                                            digital registrada.
                                        </span>
                                    </div>
                                </div>

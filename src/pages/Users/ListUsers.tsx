...
const ListUsers: React.FC = () => {
  ...
  const handleAction = async (action: string, item: User) => {
    switch (action) {
      ...
      case "delete":
        ...
        break;
      case "profile":
        navigate(`/user/profiles/${item.id}`);
        break;
      case "address":
  ...
  const deleteUser = async (item: User) => {
    ...
    if (result.isConfirmed) {
      ...
      if (success) {
        await Swal.fire("Eliminado", "El registro se ha eliminado", "success");
        fetchData();
      } else {
  ...
  const actions: Action[] = [
    ...
    { name: "digital-signature", label: "Digital Signture" },
    { name: "devices", label: "Devices" },
    { name: "passwords", label: "Passwords" },
    { name: "security-questions", label: "Security" },
